...
`docs/agent-handoffs/TASK_4_SCHEMA_DECISIONS.md`.
...
### `treatment` — no `total_amount` column

`total_amount` is intentionally omitted from the `treatment` table.
It is computed at API serialization time:
...
Storing a derived value that depends on two mutable columns creates drift
risk.
...
CREATE UNIQUE INDEX irrigation_event_rachio_event_id_uniq
    ON irrigation_event (rachio_event_id)
...
| `da5a66c32ed2` | `create_base_tables` | All 9 non-hypertable tables |
